...
from safety_gymnasium.tasks.safe_vla import AbstractSPOCTask
...
def start_worker(worker, agent_class, agent_input, device, tasks_queue, results_queue, tasks_list, max_restart_times):
    ...
    try:
        ...
        worker.distribute_evaluate(agent, tasks_queue, results_queue, tasks_list, max_restart_times)
    finally:
...
class OnlineEvaluatorWorker:
    ...
    def distribute_evaluate(
        self, agent: AbstractAgent, tasks_queue: mp.Queue, results_queue: mp.Queue, tasks_list: List[Dict[str, Any]], max_restart_times: int
    ):
        ...
        while True:
            ...
            restart_task = None
            empty_flag = False
            starting_pot_error_flag = False
            while restart_times < max_restart_times:
                try:
                    task = self.task_sampler.next_task(sample=restart_task)
                    if task is None:
                        starting_pot_error_flag = True
                        print(f"Task {restart_task['sample_id']} cannot be sampled")
                        # add restart_task['sample_id'] to a file
                        with open(f"failed_task_ids.txt", "a") as f:
                            f.write(f"{restart_task['sample_id']}\n")
                        f.close()
                        break
                ...
                success = sample_result["metrics"]["success"]
                if not bool(success > 0.1) or sample_result["metrics"]["cost_robot"] > 30:
                # if not bool(success > 0.1):
                    restart_times += 1
                    task_sample_id = task.task_info["eval_info"]["sample_id"]
                    for task_tmp in tasks_list:
                        if task_tmp["sample_id"] == task_sample_id:
                            restart_task = task_tmp
                            break
                    restart_task = task_tmp
                else:
