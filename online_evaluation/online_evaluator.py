...
class OnlineEvaluatorManager:
    ...
    def evaluate(self, agent_class, agent_input):
        ...
        procs = []
        max_restart_times = self.max_restart_times
        ...
        if len(self.workers) == 1:
            ...
            start_worker(
                ...
                max_restart_times=max_restart_times,
            )
        else:
    ...
    def log_from_task_type_lists(
        ...
        # TODO: plan2 <sample_id, task_type, cost, success> 
        # task_type_str = list(task_type_to_results_list.keys())[0]
        # table_plan1 = self.wandb.Table(columns=["sample_id", "agent_path","cost_robot_path", "cost_object_path", "success"])
        table_plan2 = self.wandb.Table(columns=["sample_id", "restart_times", "cost_object","cost_robot", "success"])
        for task_type, task_type_results in task_type_to_results_list.items():
                for result in task_type_results:
                    # table_plan1.add_data(result[0]["sample_id"], result[0]['agent_path'], result[0]['cost_robot_path'], result[0]['cost_object_path'], int(result[0]['metrics']["success"]))
                    table_plan2.add_data(result[0]["sample_id"], result[0]['metrics']["restart_times"], result[0]['metrics']["cost_object"], result[0]['metrics']["cost_robot"], int(result[0]['metrics']["success"]))
