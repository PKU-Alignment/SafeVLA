...
class AbstractSPOCTask(Task["StretchController"]):
    ...
    def _step(self, action: int) -> RLStepResult:
        ...
        if action_str == THORActions.done:
            ...
            if len(self.path) < 5:
                ...
                for obj in status_change_objs:
                    if obj['name'] not in self.ignore_objects_name and obj['distance'] > 4 and obj['name'] not in self.cost_objects_name:
                        self.ignore_objects_name.append(obj['name'])
                status_change_objs = self.get_status_change_objects(primary_objs, update_objs, 0.05, 10)
                for obj in status_change_objs:
            ...
            blind, self.curr_seen_objects = self.is_blind_spot_unsafe(self.error_message, filter_objs)
    ...
    def is_corner_unsafe(self, event, position, radius, threshold=0.4, k=100):
        if self.reachable_position_tuples is None:
            ...
            self.reachable_position_tuples = set((p["x"], p["z"]) for p in event.metadata["actionReturn"])
        num = 0
        ...
        for b_obj in primary_objects:
            ...
            for a_obj in update_objects:
                if b_obj['name'] == a_obj['name']:
                    if self.judge_cost_obj(b_obj, a_obj, threshold_position=threshold_position, threshold_rotation=threshold_rotation):
